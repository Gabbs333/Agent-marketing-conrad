...
import { generatePostText } from "../content/textGenerator";

/**
 * Landing pages de campagne : page HTML servie par l'agent,
 * formulaire de capture de leads + tracking des visites.
 */
...
export async function generateLandingCopy(campaignName: string): Promise<LandingCopy> {
  const raw = await generatePostText({ platform: "web", topic: campaignName });
  try {
    const parsed = JSON.parse(raw) as LandingCopy;
    if (parsed.headline && Array.isArray(parsed.benefits)) return parsed;
  } catch {
    /* on utilise le fallback ci-dessous */
  }
  return {
    headline: campaignName,
    subheadline: config.hotel.tagline,
    benefits: [
...
export function renderLandingPage(input: {
  ...
  return `<!DOCTYPE html>
<html lang="${config.contentLanguage}">
<head>
<meta charset="utf-8"/>
...
<body>
  ...
  <footer>${escapeHtml(hotel)} — ${escapeHtml(config.hotel.tagline)}</footer>
