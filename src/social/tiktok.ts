...
async function toBuffer(pathOrUrl: string): Promise<Buffer> {
  if (/^https?:\/\//.test(pathOrUrl)) {
    const res = await fetch(pathOrUrl);
    if (!res.ok) throw new Error(`Téléchargement du média échoué (${res.status})`);
...
export async function uploadAndPublishVideo(input: {
  videoPath: string;
  caption: string;
}): Promise<PublishResult> {
  if (config.demoMode || !config.tiktok.accessToken) {
  ...
  const init = await httpJson(`${TIKTOK_API}/post/publish/video/init/`, {
    method: "POST",
    headers: authHeaders(),
    body: JSON.stringify({
      post_info: {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    const st = await httpJson(`${TIKTOK_API}/post/publish/status/fetch/`, {
      method: "POST",
      headers: authHeaders(),
      body: JSON.stringify({ publish_id: publishId }),
    });
    const status = st.data?.status;
    if (status === "PUBLISH_COMPLETE") return { id: publishId, scheduled: false };
    if (status === "FAILED") {
...
export async function publishPhoto(input: {
  ...
  const init = await httpJson(`${TIKTOK_API}/post/publish/photo/init/`, {
    method: "POST",
    headers: authHeaders(),
    body: JSON.stringify({
      post_info: {
  ...
  const attach = await httpJson(`${TIKTOK_API}/post/publish/photo/init/`, {
    method: "POST",
    headers: authHeaders(),
...
export function buildAuthUrl(redirectUri: string, scopes: string[]): string {
  const scope = encodeURIComponent(scopes.join(","));
  const redirect = encodeURIComponent(redirectUri);
  return `https://www.tiktok.com/v2/auth/authorize/?client_key=${config.tiktok.clientKey}&scope=${scope}&response_type=code&redirect_uri=${redirect}`;
}
...
export async function getAccessToken(code: string, redirectUri: string): Promise<string> {
  ...
  const data = await httpJson(`${TIKTOK_API}/oauth/token/`, {
    method: "POST",
  ...
  if (!data.access_token) throw new Error(`OAuth TikTok échoué : ${JSON.stringify(data).slice(0, 300)}`);
...
export async function getUserInfo(token?: string): Promise<{
  ...
  const t = token ?? config.tiktok.accessToken;
  if (!t) throw new Error("TIKTOK_ACCESS_TOKEN manquant");
  const data = await httpJson(`${TIKTOK_API}/user/info/?fields=open_id,display_name`, {
